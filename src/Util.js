...
const rotateWorldSpace = (object, r) => {
  let euler = new THREE.Euler(r.x * PI/180, r.y * PI/180, r.z * PI/180);
  let rotateMatrix = new THREE.Matrix4();
  rotateMatrix.makeRotationFromEuler(euler);
  object.applyMatrix(rotateMatrix);
}

const translateWorldSpace = (object, v) => {
  let transMat = new THREE.Matrix4();
  transMat.makeTranslation(v.x, v.y, v.z);
  object.applyMatrix(transMat);
};

const scaleWorldSpace = (object, s) => {
  let scaleMat = new THREE.Matrix4();
  scaleMat.makeScale(s.x, s.y, s.z);
  object.applyMatrix(scaleMat);
};

const v3 = (...args) => {
  if (args.length === 1) {
    return new THREE.Vector3(args[0], args[0], args[0]);
  }
...
const min = (arr, f) => {
  let m = arr.map(f);
  return arr[m.indexOf(Math.min(...m))];
}
...
const randColor = () => {
  let keys = Object.keys(ColorKeywords)
  let filtered = keys.filter(key => {
    let hex = ColorKeywords[key];
    let r = (hex >> 16) & 0xFF;
    let g = (hex >> 8) & 0xFF;
    let b = hex & 0xFF;
    return (r + g + b) / 3 > 127;
  })
  let randKey = filtered[Math.floor(Math.random() * filtered.length)];
  return randKey;
}
